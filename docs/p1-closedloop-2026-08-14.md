...
| 项 | 状态 | 说明 |
| --- | --- | --- |
| P1 #5 双壳二选一 | 未做 | `client/`（托盘）与 `desktop/`（WPF）仍并存，需废弃其一 |
| annotate 确认的 dry-run 预览呈现 | 部分 | 确认卡显示参数；尚未把宿主 dry-run 预览摘要渲染进卡片 |
| P2 真安装器（MSIX/Inno + 内嵌运行时） | 未做 | 仍依赖预装 Python + .NET |
| P2 连接自愈可见化 | 部分 | 陈旧注册清理已有；端口冲突/桥崩溃自动恢复未呈现 |
| P2 陌生环境实测 | 未做 | 需要未接触过项目的人从零安装一遍 |
| 交接遗留：两个窗未挂接 | 未做 | 需人工确认图纸问题（见 handover 文档） |
| 交接遗留：AutoCAD 2014–2024 实机连接验证 | 未做 | 已纳入一键连接编排，缺实机记录 |
