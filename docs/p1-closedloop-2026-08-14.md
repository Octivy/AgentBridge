...
| P2 真安装器（MSIX/Inno + 内嵌运行时） | 已完成 | Inno Setup 6：自包含 .NET 发布 + 内嵌 Python 运行时 + 中文向导；自举测试通过（安装→启动→健康→卸载） |
| P2 连接自愈可见化 | 已完成 | 事件日志 + 桥进程守护（崩溃自动重启/预算/端口归因）+ 监控页事件流 |
| P2 陌生环境实测 | 未做 | 需要未接触过项目的人从零安装一遍（安装器已可交付） |
| 交接遗留：两个窗未挂接 | 已关闭 | 用户决策：取消人工核对，连接验证通过后改用新图纸测试 |
