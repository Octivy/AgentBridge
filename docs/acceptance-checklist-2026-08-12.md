...
| 9 | 多软件适配器 | Blender 实机验证（6 工具，含可见 GUI 窗口建别墅）；SketchUp 实机验证（2 工具，SketchUp 2025）；Rhino Python（契约测试通过，待 Rhino 重装后实机）；AutoCAD cadmcp 13 工具 | ⏳ 仅 Rhino |
| 10 | 质量门禁 | pytest 215 通过 + 4 子测试；ruff 全绿；桌面端 Release 构建通过；打包脚本可产出便携包 | ✅ |
